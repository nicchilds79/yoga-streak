const POSES = [...STANDING_POSES, ...BANDHAS];
...
let poseQueue = Array.isArray(rotationSaved.queue) ? rotationSaved.queue.filter(name => typeof name === "string" && POSES.some(p => p.sanskrit === name)) : [];
...
function makeQuestions() {
  while (poseQueue.length < GOAL) {
    ...
    poseQueue.push(...shuffle(fresh).map(p => p.sanskrit), ...shuffle(recent).map(p => p.sanskrit));
  }
...
function renderQuestion() {
  ...
  if (mode === 0) {
    ...
    type = "TRANSLATE"; question = "What does this Sanskrit term mean?";
...
function renderLibrary() {
  ...
  $("pose-list").innerHTML = POSES.map((p, index) => {
    ...
    const labels = p.kind === "bandha"
      ? ["Body focus", "Aim", "Traditional context", "Starting position", "Action", "Watchpoints", "Foundation practice", "Advanced practice", "Contraindications", "Memory hook", "Teaching cue"]
      : ["Muscle focus", "Aim of pose", "Benefits", "Starting position", "Action", "Watchpoints", "Beginner modification", "Advanced option", "Contraindications", "Memory hook", "Teaching cue"];
    const values = [p.focus, p.aim, p.benefits, p.start, p.action, p.watch, p.beginner, p.advanced, p.contra, p.hook, p.cue];
    const details = labels.map((label, i) => `<dt>${label}</dt><dd>${values[i]}</dd>`).join("");
    return `${heading}<article class="pose-card"><button class="pose-summary" data-expand="${index}" aria-expanded="false"><span class="pose-symbol">${p.emoji}</span><span class="pose-copy"><strong class="pose-sanskrit">${p.sanskrit}</strong><span class="pose-english">${p.english}</span><span class="pose-pronunciation">${p.phonetic}</span></span><b>＋</b></button><div class="pose-details hidden" id="pose-details-${index}">${visual}<button class="speak" data-pose="${p.sanskrit}">🔊 Hear pronunciation</button><dl>${details}</dl></div></article>`;
  }).join("");
