...
function renderLibrary() {
  ...
  $("pose-list").innerHTML = POSES.map((p, index) => {
    ...
    return `${heading}<article class="pose-card"><button class="pose-summary" data-expand="${index}" aria-expanded="false"><span class="pose-symbol">${p.emoji}</span><span><strong>${p.sanskrit}</strong><small>${p.english} · ${p.phonetic}</small></span><b>＋</b></button><div class="pose-details hidden" id="pose-details-${index}">${visual}<button class="speak" data-pose="${p.sanskrit}">🔊 Hear pronunciation</button><dl><dt>Muscle focus</dt><dd>${p.focus}</dd><dt>Aim of pose</dt><dd>${p.aim}</dd><dt>Benefits</dt><dd>${p.benefits}</dd><dt>Starting position</dt><dd>${p.start}</dd><dt>Action</dt><dd>${p.action}</dd><dt>Watchpoints</dt><dd>${p.watch}</dd><dt>Beginner modification</dt><dd>${p.beginner}</dd><dt>Advanced option</dt><dd>${p.advanced}</dd><dt>Contraindications</dt><dd>${p.contra}</dd><dt>Memory hook</dt><dd>${p.hook}</dd><dt>Teaching cue</dt><dd>${p.cue}</dd></dl></div></article>`;
  }).join("");
...
if ("serviceWorker" in navigator) navigator.serviceWorker.register("sw-v5.js");
